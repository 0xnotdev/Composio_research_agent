...
def render_case_study(records: list[dict[str, Any]], analytics: dict[str, Any], verification: dict[str, Any] | None, generated_at: str) -> str:
    ...
    raw_dataset = json.dumps(records, ensure_ascii=False).replace("</", "<\\/")
...
<section><h2>What the portfolio says</h2><p>Metrics are calculated directly from the embedded dataset. Unknown and conflicting fields are retained rather than smoothed away: {coverage.get('conflicting_fields',0)} fields remain conflicting.</p><div class="two"><div class="card"><h3>Easy wins</h3><p>Self-serve, technically ready apps with a moderate/broad API surface and no official vendor MCP.</p><ul>{wins}</ul></div><div class="card"><h3>Needs outreach</h3><p>Strong API surface, but partner or sales access constrains a toolkit launch.</p><ul>{outreach}</ul></div></div></section>
